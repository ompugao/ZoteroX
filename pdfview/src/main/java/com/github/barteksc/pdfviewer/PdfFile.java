...
//    private PdfDocument pdfDocument;
    ...
    PdfFile(PdfiumSDK pdfiumSDK, FitPolicy pageFitPolicy, Size viewSize, int[] originalUserPages,
...
//        this.pdfDocument = pdfDocument;
    ...
    private void setup(Size viewSize) {
        if (originalUserPages != null) {
            ...
            pagesCount = pdfiumSDK.getPageCount();
        }

        for (int i = 0; i < pagesCount; i++) {
            Size pageSize = pdfiumSDK.getPageSize(documentPage(i));
    ...
    public void renderPageBitmap(Bitmap bitmap, int pageIndex, Rect bounds, boolean annotationRendering) {
        int docPage = documentPage(pageIndex);
        pdfiumSDK.renderPageBitmap(bitmap, docPage,
...
//            return null;
//        }
        return pdfiumSDK.getDocumentMeta();
    }
...
////        }
        return pdfiumSDK.getTableOfContents();
    }

    public List<Link> getPageLinks(int pageIndex) {
        int docPage = documentPage(pageIndex);
        return pdfiumSDK.getPageLinks(docPage);
    }
...
//        return pdfiumCore.mapRectToDevice(pdfDocument, docPage, startX, startY, sizeX, sizeY, 0, rect);
        return pdfiumSDK.mapPageCoordinateToDevice(docPage, startX, startY, sizeX, sizeY, 0, rect);
    }
